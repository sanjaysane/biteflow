...
def test_referral_unknown_and_own_code(db):
    ref = MK.get_or_create_referral(db, COOK, COOK, reward_amount=2.00)
    assert (
        MK.apply_referral(db, COOK, CUST, "NOPE-1234", 8.50)["reason"] == "unknown_code"
    )
    assert MK.apply_referral(db, COOK, COOK, ref["code"], 8.50)["reason"] == "own_code"
